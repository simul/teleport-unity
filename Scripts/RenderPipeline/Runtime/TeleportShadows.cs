...
public class TeleportShadows
{
	...
	public void RenderScreenspaceShadows(ScriptableRenderContext context, Camera camera, CullingResults cullingResults, PerFrameLightProperties perFrameLightProperties, int cascadeCount)
	{
		...
		if (fullscreenMesh == null)
			fullscreenMesh = teleport.RenderingUtils.CreateFullscreenMesh(camera.fieldOfView * camera.aspect, camera.fieldOfView, camera.farClipPlane);
		buffer.DrawMesh(fullscreenMesh, Matrix4x4.identity, shadowMaterial);
